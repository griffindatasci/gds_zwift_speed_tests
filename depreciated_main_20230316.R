...
# - repeat with Strava segment times




read_sheet("https://docs.google.com/spreadsheets/d/1IFuQUhQ1Ek6JTa5X2ZJpFEfSUxeqYrXXT58_0c2Hp1k",
           sheet="strava_segment_times") %>% 
  data.table() -> 
  strava_times

strava_times[, mountain_route := ifelse(nchar(mountain_route)==5, paste0("0;", mountain_route), mountain_route)]
strava_times[, mountain_route := strava_times[, tstrsplit(mountain_route, ";")][, c(as.numeric(V1)*3600 + as.numeric(V2)*60 + as.numeric(V3))]]


strava_times[
  tests[, .(test_id, power, frame, wheel)], on="test_id"][
  , .(test_id, power, frame, wheel, mountain_route)][
    crossed[, .(frame, power, wheel, seconds)], on=c("power", "frame", "wheel")][!is.na(mountain_route)] -> 
  strava_comparison

strava_comparison[, plot(mountain_route~seconds)]
strava_comparison[, mountain_route-seconds]
